...
total_single_sided_mapped = int(output_dict["total_single_sided_mapped"])
total_single_sided_mapped_str = format(total_single_sided_mapped, ",d")
percent_LOWMAPQ = round(total_single_sided_mapped*100 / total_reads, 2)
table.append(["Low Mapping Qualityr",total_single_sided_mapped_str, f"{percent_LOWMAPQ}%"] )
...
with open(args.o, 'w', newline='') as csvfile:
    ...
    csv_writer.writerow(["Description", "Count", "Percentage"])
    csv_writer.writerows(table)

print(f"Results have been saved to {args.o}")
